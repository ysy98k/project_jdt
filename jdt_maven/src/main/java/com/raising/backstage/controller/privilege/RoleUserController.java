 ...
 */
@RequestMapping("/raising/backstage/privilege/roleUser")
@RestController(value = "roleUserController")
public class RoleUserController extends BaseController {
    ...
    public ModelAndView init(HttpServletRequest request, HttpServletResponse response){
        return new ModelAndView("/raising/backstage/privilege/RoleUser");
    }
     ...
     * @return
     */
    @RequestMapping(value="getUserSelect.do",method = RequestMethod.GET)
    public ModelAndView getD(HttpServletRequest request, HttpServletResponse response){
        return new ModelAndView("/raising/backstage/privilege/UserSelect");
    }
    ...
    private JSONObject pushDefaultGroup(JSONObject paramJson){
        ...
        if(defaultGroupUserIds.size() > 0){
            ...
            try {
                roleUserService.bindUser(bindUserJson.toJSONString(),Integer.parseInt(request.getSession().getAttribute("userId").toString()));
            } catch (Exception e) {
                e.printStackTrace();
                returnInfo.put("status", Constants.EXECUTE_FAIL);
     ...
     * @return
     */
    @RequestMapping(value = "getUserSelectRows.do",method = RequestMethod.POST)
    public JSONObject getUserSelectRows(String ajaxParam){
        ...
        try{
            ...
            //3.移除当前登陆用户的UserId
            userIds.remove(request.getSession().getAttribute("userId"));
    ...
    public JSONObject bindUser(String ajaxParam){
        ...
        try{
            MultipleDataSource.setDataSourceKey(CSVUtils.ConstantsUtil.AASCCS_DATA_SOURCE);
            try {
                returnInfo = roleUserService.bindUser(ajaxParam,Integer.parseInt(request.getSession().getAttribute("userId").toString()));
            } catch (Exception e) {
                e.printStackTrace();
                returnInfo.put("status", Constants.EXECUTE_FAIL);
